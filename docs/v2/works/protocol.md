...
- **plugin 说了算。** 只有它知道自己有几种形态、当下哪种成立（比如对话形态要求
  那条切轮次的规则能用）；
- 有多种形态的 plugin **自己提供切换入口**（块里的一个控件），画布不替它摆，也不
  替它决定；
- 用户切过之后，**形态作为块的状态跟布局一起持久化**（v1 已有「块参数随布局存」
  的机制，见 [v1 design §3.4](../../v1/works/design.md)），重入现场还是上次那个形态。
  **画布只负责存和取，不解释它。**

画布层因此完全不需要认识 provider 这个词：它拿到的是「这个块要显示这扇窗 / 这棵树 /
这串消息」，至于为什么是它，是 plugin 的事。
