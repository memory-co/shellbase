...
```
claude:///workspace/proj?as=chat
└──┬──┘ └──────┬───────┘   └─┬─┘
 协议        路径           形态
 做什么      在哪儿做       用哪种方式上手

协议 ──by── plugin ──跑在── provider ──背后是── *muxd 组件 / 内置能力
```
...
| **网页浏览器** | 一个页面 | 点、填、拖 | webmuxd，或纯前端 iframe |
...
## 4. 形态怎么选，以及同一个会话的多个视图

选形态的优先级，从高到低：

1. URI 里显式写了 `?as=<provider>`；
2. 这个块上次用的形态（存在布局里，跟着 window 一起持久化）；
3. plugin 的 `default_provider`；
4. 它只支持一种形态时的那一种。
...
4. 对话形态：把同一个 session 的 `send()` / `capture()` 交给对话 provider，
   外加一条「以 shell 提示符为界切轮次」的规则。
