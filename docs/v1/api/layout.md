...
- `200` 返回上述对象；
- `404 {"error":"no_layout"}` = 首次使用，前端渲染默认单块布局（一个启动页）。

## PUT /api/layout

全量覆盖写。请求体即布局对象，`version` 必须为**当前版本 + 1**：

- 成功：`204`，服务端落盘（原子写）并向 `watch` 广播；
...
布局变更广播通道，协作的实时性来源。连接后：

```json
// 服务端 → 客户端，每次 PUT 成功后推送
{ "type": "layout_updated", "version": 6 }
```

- 客户端收到版本号大于本地的通知即 `GET /api/layout` 拉新树、diff 重渲染（uri 未变的块 iframe 原地保留，不闪断）；
